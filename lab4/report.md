...
В разделе Ingress processing добавлен метод, отвечающий за пересылки пакетов и условие проверки `ipv4 headers`.
...
Проверим заголовок `header myTunnel_t `, который включает id протокола и коммутатора приема.
...
В разделе Parsers добавим состояние `state parse_myTunnel`, а также добавим в оператор select. 
```c
/*************************************************************************
*********************** P A R S E R  ***********************************
*************************************************************************/
...
```
> В разделе Ingress processing добавим новое действие `action myTunnel_forward(egressSpec_t port)`, а также новую таблицу `table myTunnel_exact`, а также напишем условие проверки: если ip задан правильно, а тоннель нет, то отправляем пакеты без тоннеля, если все корректно, то используем тоннель.
...
```
> В разделе Deparsers добавим преобразование пакетов туннелирования.
